...
namespace ApiCatalogoJogos.Repositories
{
    ...
    {
        private static Dictionary<Guid, Jogo> jogos = new Dictionary<Guid, Jogo>() //estático para simular um banco de dados
        {
        ...
        } // skip para pular a quantidade de registro

        public Task<Jogo> Obter(Guid id)
        {
            if (!jogos.ContainsKey(id))
                return Task.FromResult<Jogo>(null); // se não existir não retorna

            return Task.FromResult(jogos[id]); //Se existir retorna o jogo
        }
